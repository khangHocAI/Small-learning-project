...
class Pipe():
    ...
    def move(self, score):
        self.x -= self.VEL + int(0.1*score)
        
        if self.move_direction:
            self.height_move += 1
        else:
            self.height_move -= 1
        if self.height_move > 10:
            self.move_direction = False
        elif self.height_move < -10: 
            self.move_direction = True
        self.top -= self.height_move
        self.bottom -= self.height_move
...
def eval_genomes(genomes, config):
    ...
    while run:
        ...
        for x, bird in enumerate(birds):
            bird.move()
            ge[x].fitness += 0.1
            out = nets[x].activate((bird.y, abs(bird.y - pipes[pipe_ind].height), abs(bird.y - pipes[pipe_ind].bottom)))
            if out[0] > 0.5:
                bird.jump()
        for pipe in pipes:
            for x, bird in enumerate(birds):
        ...
        for r in rem:
            pipes.remove(r)
        for x,bird in enumerate(birds):
            if bird.y + bird.img.get_height() >= 690 or bird.y < 0:
                birds.pop(x)
                nets.pop(x)
                ge.pop(x)     
